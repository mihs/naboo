...
module.exports = function(options, done) {
  ...
  try {
    debug("Loading configuration %s", path.join(configs, config.env))
    require(path.join(configs, config.env)).call(config);
  }
  catch (ex) {
    debug("Failed to load %s", path.join(configs, config.env))
  }
